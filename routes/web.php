...
Route::get('/images/{path}', function ($path) {
    $localPath = public_path('images/' . $path);
    ...
    // Proxy from dev site
    $remoteUrl = "https://dev.trainer-workout.com/images/" . $path;

    try {
        $remoteResponse = Http::get($remoteUrl);

        if ($remoteResponse->ok()) {
            return Response::make($remoteResponse->body(), 200, [
                'Content-Type' => $remoteResponse->header('Content-Type'),
                'Cache-Control' => 'no-cache',
            ]);
        }
    } catch (\Exception $e) {
        // Log it if you want: Log::warning("Image proxy failed: " . $e->getMessage());
    }
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/trainer/exercise-chat/{exerciseId}', [AIWorkoutController::class, 'getExerciseChat'])->name('exerciseChat.get');
    Route::post('/trainer/exercise-chat/{exerciseId}', [AIWorkoutController::class, 'sendExerciseChat'])->name('exerciseChat.send');
...
Route::get(__('routes./Workout/PrintWorkout/') . "android/{workout}", [WorkoutsController::class, 'PrintWorkoutAndroidPDF']);
...
Route::get('membership-cancellation', [MembershipsController::class, 'indexMembershipCancellation'])->name('membership-cancellation')->middleware('auth');
