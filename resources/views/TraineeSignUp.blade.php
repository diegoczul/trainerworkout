    ...
    <div class="wrapper accountRoot">
        ...
        <div class="accountAction_container">
            {{ Form::open(array('url' => Lang::get("routes./Trainee/SignUp"), "id"=>"login_form")) }}
                ...
                <input type="text" name="lastName" id="lastName" required placeholder="{{ Lang::get("content.Last Name") }}" value="{{ (isset($invite) and ($invite)) ? $invite->lastName : request()->old('lastName') }}" />
                <label for="email">{{ Lang::get("content.Email") }}</label>
                <input type="text" name="email" id="email" required placeholder="{{ Lang::get("content.Email") }}" value="{{ (isset($invite) and ($invite)) ? $invite->email : request()->old('email') }}" />
