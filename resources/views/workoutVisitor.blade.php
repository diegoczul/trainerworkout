        ...
        <div class="wrapper">
            <div class="widgets fullwidthwidget shadow">
                    ...
                    @foreach($exercises as $exercise)
                            ...
                            <div class="dayitemsdetails clearfix" exercise="">
                                <div class="dayworkouts clearfix">
                                    ...
                                    <div class="workout_image_container">
                                        <ul>
                                            <li>
                                                <img src="/{{ Helper::image($exercise->exercises->image) }}"
                                            ...
                                            <li>
                                                ...
                                                @if(count($customImages) > 0)
                                                    ...
                                                    <img id="customImage{{ $exercise->id }}"
                                                         src="/{{ Helper::image($lastImageAdded->image) }}"
                                                         alt="Trainer Workout">
                                                @else
                                ...
                                <div class="dayworkoutstable">
                                    <table class="tabulardata setsInformation exerciseTable_{{ $exercise->id }}"
                                        ...
                                        <tr>
                                            ...
                                            <th class="filters">
                                                <label>
                                                    <input type="checkbox"
                                                           onclick='exerciseCompleted({{ $exercise->id }},$(this)); return false;'
                                                           value="Yes"/>
                     ...
                     {{ ($set->weight == "" ? 0 : Helper::formatWeight($set->weight)." Lbs") }}
                                                    ...
                                                    <div class="weight_input edit {{ ($set->completed == 1 ? "hide" : "") }}">
                                                        <input name="set_weight[{{ $set->id }}]" type="text"
                                                               class="inputbox-small center input edit weight lbs_{{$sum_ex}}"
                                                               value="{{ ($set->weight == "" ? 0.0 : Helper::formatWeight($set->weight)) }}"/>
                                                ...
                                                <!-- Repetitions -->
                                                <td>
                                                    <span class="view reps {{ ($set->completed == 1 ? "" : "hide") }}">{{ $set->reps }}</span>
                                                    <input name="" type="text"
                                                           class="inputbox-small center input edit reps  reps_input {{ ($set->completed == 1 ? "hide" : "") }}"
                                                           value="{{ $set->reps }}"/>
                                                    <input name="" type="text"
                                                           class="inputbox-small center input edit rest  reps_input hide"
                                                           value="{{ $set->rest }}"/>
                                                    <input name="" type="text"
                                                           class="inputbox-small center input edit idSet reps_input hide"
                                                           value="{{ $set->id }}"/>
                                                    <input name="" type="text"
                                                           class="inputbox-small center input edit idExercise reps_input hide"
                                                           value="{{ $exercise->id }}"/>
                                                </td>
                                                <td class="filters">
                                                    <label>
                                                        <input name="set_completed[{{ $set->id }}]"
                                                               {{ ($set->completed == 1 ? "checked='checked'" : "") }} {{ ($set->completed == 1 ? "disabled='disabled'" : "") }} onClick="completeSingleSet({{ $set->id }},$(this))"
                                        ...
                                        <tr>
                                            <td></td>
                                            <td>
                                                <a href="javascript:void(0)" class="workout_table_btn"
                                                   onClick='saveAllAddNewSets($(this),{{ $exercise->id }})'>+ Add
                                                    Set</a>
                                            </td>
                                            <td>
               ...
               <div class="measure_kg_lbs">Kg</div> -->
                                            ...
                                            <td colspan="2">
                                                <a href='javascript:void(0)' status="hide"
                                                   onclick='showMore("{{ $exercise->id }}",$(this))'
                                                   class="workout_table_btn"><i class="fa fa-clock-o"></i>
                                                    <span>show</span> history</a>
                                            </td>
                                        </tr>
                                    </table>
                                </div>
                            ...
                            <div class="dayitemsdetails clearfix" exercise="">
                                <div class="dayworkouts clearfix">
                                    ...
                                    <ul>
                                        ...
                                        <li>
                                            @if($exercise->exercises->image2 != "")
                                                <a href="//fb/ViewExercise//" class="fancy"><img
                                                            src="/{{ $exercise->exercises->image2 }}"
                                                            alt="Trainer Workout - "></a>
                                            @endif
                                        </li>
                                    </ul>
                                </div>
                                    ...
                                    <table class="tabulardata setsInformation exerciseTable_{{ $exercise->id }}"
                                        ...
                                        @foreach($sets as $set)
                                            ...
                                            <tr class="set_{{ $exercise->id }} {{ ($counter >= count($sets) - $exercise->sets ? "last" : "hide" ) }}">
                                                ...
                                                <td>
                                                    ...
                                                    <div class="weight_input edit {{ ($set->completed == 1 ? "hide" : "") }}">
                                                        <input name="set_heart_beat[{{ $set->id }}]" type="text"
                                                               class="inputbox-small center input edit heart_beat lbs_{{$sum_ex}}"
                                                               value="{{ ($set->bpm == "" ? 0 : $set->bpm) }}"
                                                               style="margin-right:-35px !important;padding-right: 35px !important;"/>
                                                ...
                                                <td>
                                                    ...
                                                    <div class="weight_input edit {{ ($set->completed == 1 ? "hide" : "") }}">
                                                        <input name="set_speed[{{ $set->id }}]" type="text"
                                                               class=" center input edit speed {{ ($set->completed == 1 ? "hide" : "") }}"
                                                               value="{{ $set->speed }}"
                                                               style="margin-right:-40px !important;padding-right: 40px !important;"/>
                                                ...
                                                <td>
                                                    ...
                                                    <div class="weight_input edit {{ ($set->completed == 1 ? "hide" : "") }}">
                                                        <input name="set_distance[{{ $set->id }}]" type="text"
                                                               class="inputbox-small center input edit distance {{ ($set->completed == 1 ? "hide" : "") }}"
                                                               value="{{ $set->distance }}"/>
                                                        <span class="edit distance unselectable">km</span>
                                                    </div>
                                                    <input name="" type="text"
                                                           class="inputbox-small center input reps_input edit rest hide"
                                                           value="{{ $set->rest }}"/>
                                                    <input name="" type="text"
                                                           class="inputbox-small center input reps_input edit idSet hide"
                                                           value="{{ $set->id }}"/>
                                                    <input name="" type="text"
                                                           class="inputbox-small center input reps_input edit idExercise hide"
                                                           value="{{ $exercise->id }}"/>
                                                </td>
                                                <td class="filters">
                                                    <label>
                                                        <input name="set_completed[{{ $set->id }}]"
                                                               {{ ($set->completed == 1 ? "checked='checked'" : "") }} onClick="completeSingleSet({{ $set->id }},$(this))"
                                                               {{ ($set->completed == 1 ? "disabled='disabled'" : "") }}  type="checkbox"
                                                               value="Yes"/>
                                                        <span class="icon"><i class="fa fa-check"></i></span>
                                                    </label>
                                                </td>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href='javascript:void(0)' status="hide"
                                                   onclick='showMore("{{ $exercise->id }}",$(this))'
                                                   class="workout_table_btn"><i class="fa fa-clock-o"></i> history</a>
                                            </td>
                                        </tr>
                                    </table>
                                </div>
