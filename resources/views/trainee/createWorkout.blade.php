...
<div class="cw wrapper">
    ...
    <div class="widget cw-header">
        <div class="ptLogoPlaceholder" style="cursor:pointer; @if(Auth::user()->activeLogo) background-image: none; @endif" onclick="window.location='{{ Lang::get("routes./Trainee/Profile") }}'">
        ...
        <div class="cw-header-description" style="padding: 0px">
            ...
            <div style="margin-bottom: 10px; max-width:100%;" class="share_content">
...
<div class="cw-exercise muscle templateMuscle exerciseTarget mainExerciseBlock" style="display:none">
    ...
    <div class="containerExercise">
        <div class="exerciseNote" style="display: inline-block; vertical-align: top;">
...
<div class="hide">
<form method="post" enctype="multipart/encode" action="{{ Lang::get("routes./Trainee/CreateWorkout") }}" id="createform" style="display:none">
@csrf
    ...
    if(exerciseGroups[group][exNumber].exercise.bodygroupId == 18){
        ...
        if (debug) console.log('checkcheck');
            ...
            for(t = 0; t < sets.length; t++){
                ...
                html +=  '<td class="tbSet repStyleChoice mode-option" onclick="muscleBoundMenu(this,'+t+')">'+dict[exerciseGroups[group][exNumber].repsType[t]]+'</td>';
                html += "</tr>";
                setNumber++;
            }
            html += "</tbdoy></table>";
...
function addToSearchResultView(exercise,index){
  var html = '';
    var favourite = "";
  if(exercise.hasOwnProperty('favorite') && exercise.favorite == 1){
        favourite = "favourite-star";
  }
  if(exercise.image !== undefined && exercise.image !== null){
  html = '<li class="elementSearch '+ favourite +'" id="search_'+exercise.elementID+'">'+
...
function autoSaveWorkout(){
  ...
  if(autoSaveExercise != JSON.stringify(exerciseGroups)){
    ...
    autoSaveExercise = JSON.stringify(exerciseGroups);
      ...
      $.ajax({
        url: "{{ Lang::get("routes./Trainee/autoSaveWorkout") }}",
        type: "POST",
...
function fillUpExercise(div,exercise){
    ...
    $("#"+div).attr("data-exercise-id", exercise.exercise.id);
...
<script>
    ...
    function openAITrainerChat(element) {
        ...
        const exerciseContainer = element.closest('.containerExercise').parentElement;
        ...
        const exerciseHeader = exerciseContainer.querySelector('.exerciseHeader');
        const exerciseNameElement = exerciseHeader.querySelector('h1, h5');
    ...
    function loadChatHistory() {
        ...
        fetch(`/trainee/exercise-chat/${currentExerciseId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
    ...
    function appendMessage(sender, message, scroll = true) {
        ...
        // Format AI messages with markdown-style formatting
        if (sender === 'ai') {
    ...
    function formatMarkdown(text) {
        ...
        text = text.replace(/^(\d+\.\s+)(.*)$/gm, '<div style="margin-left: 15px;">$1$2</div>');
        
        // Replace bullet points (- at start of line)
        text = text.replace(/^-\s+(.*)$/gm, '<div style="margin-left: 15px;">• $1</div>');
        
        // Replace multiple consecutive line breaks with single <br>
        text = text.replace(/\n+/g, '<br>');
    ...
    window.onclick = function(event) {
        const modal = document.getElementById('aiTrainerModal');
