...
<!-- ******  ADD LINK BELOW  ***** -->
                @if($workout->status == "Draft")
                    @if(isset($options) and (array_key_exists("click", $options) and $options["click"]))
                            <a class="WorkoutsHoverView" href="{{ $workout->getEditURL() }}{{ isset($client) ? "/".$client : "" }}">{{ Lang::get("content.CreateWorkout") }}</a>
                    @elseif(isset($options) and (array_key_exists("add", $options) and $options["add"]))
                            <a class="WorkoutsHoverView" href="{{ Lang::get("routes./Workouts/addWorkoutToClient") }}/{{ $workout->id }}{{ isset($client) ? "/".$client : "" }}">{{ Lang::get("content.CreateWorkout") }}</a>
                    @else
                            <a class="WorkoutsHoverView" href="/{{ $workout->getURL() }}">{{ Lang::get("content.CreateWorkout") }}</a>
                    @endif
                ...
                @else
                    ...
                    @elseif(isset($options) and (array_key_exists("add", $options) and $options["add"]))
                            <a class="WorkoutsHoverView" href="{{ Lang::get("routes./Workouts/addWorkoutToClient") }}/{{ $workout->id }}{{ isset($client) ? "/".$client : "" }}">{{ Lang::get("content.CreateWorkout") }}</a>
                    @else
                             <a class="WorkoutsHoverView" href="/{{ $workout->getURL() }}">{{ Lang::get("content.ViewWorkout") }}</a>
                    @endif
...
<!-- ******  Duplicate  ***** -->
                    ...
                    <div class="workoutsHover_BTNContainer hvr-grow {{ ($workout->status == "Draft") ? "disableDiv" : "" }}" onclick="event.cancelBubble = true;">
                        <a href="javascript:void(0)" onClick="showDuplicatePopup($(this)); return false;" class="workoutsHoverBTN {{ ($workout->status == "Draft") ? "disableButton" : "" }}">
                            <div class="workoutsHover_BTNimg">
                                <img src="/img/duplicateWorkoutIcon.svg">
                            </div>
...
<!-- ******   Select   ***** -->        
                    ...
                    <div class="workoutsHover_BTNContainer hvr-grow" onclick="event.cancelBubble = true;">
                        <a href="javascript:void(0)" class="workoutsHoverBTN" onclick="putAllWorkoutsOnSelectMode(this,event);event.stopPropagation(); ">
                            <div class="workoutsHover_BTNimg">
                                <img workoutid="{{ $workout->id }}" class="selectable" src="/img/selectableWorkoutIcon.svg">
                            </div>
                            <div class="workoutsHover_BTNtxt">
...
<!-- The end of the hover div -->
           ...
           <!-- end of the div -->          
            ...
            <div class="workout_overlay">
                ...
                <span class="workout_title" title="{{ $workout->name }}">{{ Helper::text($workout->name,70) }}</span>
...
@endif
    ...
    <a href="javascript:void(0)" onclick="callWidget('w_workouts',{{ $workouts->count()}},null,$(this),{archive: '{{ ($archive) ? "true" : "false" }}', search:'{{{ (isset($search) ? $search : "") }}}'})" class="viewMore">{{ Lang::get("content.ViewMore") }}</a>
    @endif
