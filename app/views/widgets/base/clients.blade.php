...
<!-- ******  View Workouts  ***** -->
                <div class="clientsHover_BTNContainer hvr-grow" onclick="event.cancelBubble = true;">
                    <a href="javascript:void(0)" class="clientsHoverBTN" onclick="subscribeToClient(this,{{ $client->id }})">
                        <div class="clientsHover_BTNimg">
                            <?php 
                            $userUpdate = false;
                            if($client->subscribeClient == 1){ $userUpdate = true; } else { $userUpdate = false; } 
                           
                            ?>
                            
                            <img clientid="{{ $client->id }}" class="selectable {{ ($userUpdate) ? "objectSelected" : "" }}" src="/img/{{ ($userUpdate) ? "selectedWorkoutIcon" : "selectableWorkoutIcon" }}.svg"  {{ ($userUpdate) ? "selected='selected'" : "" }}>
                        </div>
                ...
                <div class="clientsHover_BTNContainer hvr-grow" >
                    <a href="{{ Lang::get("routes./Client/").$client->id."/".(( $client->user and $client->user->getCompleteName() != "" ) ? $client->user->getCompleteName() :  ($client->user ? $client->user->email : "Client")) }}" class="clientsHoverBTN">
                        <div class="clientsHover_BTNimg">
                            <img src="/img/svg/goIn.svg">
                        </div>
                        <div class="clientsHover_BTNtxt">
...
<!-- *****  Delete   ***** -->
                <div class="clientsHover_BTNContainer hvr-grow" onclick="event.cancelBubble = true;">
                    <a href="javascript:void(0)" onClick="deleteClients({{ $client->id }},$(this)); arguments[0].stopPropagation(); return false;" class="clientsHoverBTN">
                        <div class="clientsHover_BTNimg">
                            <img src="/img/deleteWorkoutIcon.svg">
                        </div>
                        <div class="clientsHover_BTNtxt">
...
<!-- ******   Select   ***** -->                   
                <div class="clientsHover_BTNContainer hvr-grow" onclick="event.cancelBubble = true;">
                    <a href="javascript:void(0)" class="clientsHoverBTN" onclick="putAllWorkoutsOnSelectMode(this,event);event.stopPropagation(); ">
                        <div class="clientsHover_BTNimg">
                            <img clientid="{{ $client->id }}" class="selectable" src="/img/selectableWorkoutIcon.svg">
                        </div>
                        <div class="clientsHover_BTNtxt">
        ...
        <div class="client_overlay">
            <div class="client_baseInfo">
                <img alt="{{$client->name}}" src="/{{ Helper::image(($client->user) ? $client->user->thumb : "") }}"/>
...
function subscribeToClient(object,id){
    ...
    if (typeof subscribeToClient !== typeof undefined && subscribeToClient !== false) {
         ...
         $(object).find(".selectable").attr("src","/img/selectableWorkoutIcon.svg");
        ...
        object2.attr("src","/img/selectedWorkoutIcon.svg");
...
function hideHover() {
    ...
    <div class="btmbuttonholder">
        ...
        <a href="javascript:void(0)" onclick="callWidget('w_clients',{{ count($clients) }},null,$(this))" class="viewMore">{{ Lang::get("content.MoreClients") }}</a>
    </div>
    @endif
@else
