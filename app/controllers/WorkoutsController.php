...
class WorkoutsController extends \BaseController {
	...
	public function index()
	{
		...
		if(Input::has("userId")){
			$permissions = Helper::checkPremissions(Auth::user()->id,Input::get("userId"),"w_workouts");
			if($permissions["view"]){
		...
		return View::make("widgets.base.workouts")
	...
	public function indexWorkoutsLibrary()
	{
		...
		return View::make("widgets.base.workoutsLibrary")
			->with("workouts",$workouts)
			->with("countArchiveWorkouts",$countArchiveWorkouts)
			->with("permissions",$permissions)
			->with("options",$options)
	...
	public function searchWorkout(){
		...
		if(Input::has("arrayData")){
			$arrayData = json_decode(Input::get("arrayData"));
			if(array_key_exists("search", $arrayData) and $arrayData["search"] != "") $search = $arrayData["search"];
			if(array_key_exists("archive", $arrayData) and $arrayData["archive"] == "true") $archive = true;
		}
		if(Input::has("pageSize")) $this->pageSize = Input::get("pageSize") + $this->pageSize;
		if(Input::has("archive") and Input::get("archive") == "true")  $archive = true;
	...
	public function indexTrendingWorkouts()
	{
		...
		if(Input::has("pageSize")) $this->pageSizeTrending = Input::get("pageSize") + $this->pageSizeTrending;

		return View::make("widgets.base.trendingWorkouts")
	...
	public function workoutSales(){
		...
		return View::make("widgets.base.workoutSales")
		->with("sales",Workouts::where("userId","=",0)->get())
		->with("total",Workouts::where("userId","=",0)->count());
	}

	public function subscribeTrainer(){
		$id = Input::get("workoutId");
		$workout = Workouts::find($id);
		$total = 0;
		if($workout){
			if(Input::get("subscribeToWorkout") == "true"){
				...
				return $this->responseJson(Lang::get("messages.SubscribedToWorkout",array("workouts"=>$total)));
			} else{
				$childWorkouts = Workouts::select("id")->where("master",$workout->id)->where("authorId",$workout->authorId)->lists("id");
				$updates = UserUpdates::where("trainerId",Auth::user()->id)->where(function($query) use($workout,$childWorkouts){ $query->orWhere("auxId", $workout->id); $query->orWhereIn("auxId",$childWorkouts); })->where("type","workout")->get();
				foreach($updates as $update){
				...
				return $this->responseJson(Lang::get("messages.NotSubscribedToWorkout",array("workouts"=>$total)));
			}
			
		} else{
			return $this->responseJsonError(Lang::get("messages.NotSubscribedToWorkout",array("workouts"=>$total)));
		}
	}

	public function indexMarket(){

		return View::make("workoutMarket");
	}

	public function shareWorkoutIndex($workoutId=""){
		$workout = Workouts::find($workoutId);

		return View::make("popups.shareWorkout")
	...
	public function addToWorkoutClient($workoutId,$clientId){
		...
		return Redirect::route("TrainerClients")->with("message",Lang::get("messages.workoutAddedToClient"));
	}
	...
	public function AddToMyWorkouts($workoutId){
		...
		return Redirect::route(Auth::user()->userType,array('userName' => Helper::formatURLString(Auth::user()->firstName.Auth::user()->lastName)))->with("message",Lang::get("messages.SharedWorkoutAdded"));
	}
	...
	public function acceptWorkoutBySharingLink($link){
				...
				if($workout){
					...
					if(Auth::check()){
						if(array_key_exists("HTTP_REFERER", $_SERVER) and $_SERVER['HTTP_REFERER'] != ""){
							return $this->responseJson(Lang::get("messages.SharedWorkoutAdded"));
						} else {
							return Redirect::route(Auth::user()->userType, array('userName' => Helper::formatURLString(Auth::user()->firstName.Auth::user()->lastName)))->with("message",Lang::get("messages.SharedWorkoutAdded"));
						}
					} else {
						return Redirect::route('home')->with("message",Lang::get("messages.SharedWorkoutAdded"));
					}
		...
		return Redirect::route(Auth::user()->userType,array('userName' => Helper::formatURLString(Auth::user()->firstName.Auth::user()->lastName)))->withError(Lang::get("messages.Oups"));
	}
	...
	public function ShareByEmail(){
		...
		if(!$validation){
			return $this->responseJsonError(Lang::get("messages.EmailNotValid"));
		}

		$workoutsString = Input::get("workoutId");
		$workoutsArray = explode(",",$workoutsString);

		$subscribe = false;


		if(Input::get("subscribeToWorkout") == "true") $subscribe = true;
		...
		foreach($workoutsArray as $workoutId){
			...
			if($workout){
				...
				if($workout->canThisWorkoutBeShared(Auth::user())){
						$user = null;
			        	$stringOfUsersToShare = Input::get("email");
			        	...
			        	foreach($arrayOfUsersToShare as $email){
			        		...
			        		if($email != ""){
				        		if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
									...
									if(Input::get("copyMe") ==  "true") $copyMe = true;
									if(Input::get("copyPrint") ==  "true") $copyView = true;
									if(Input::get("copyPrint") ==  "true") $copyPrint = true;
									if(Input::get("lock") ==  "true") $lock = true;

									

									

						        	Event::fire('shareAWorkout', array(Auth::user(),$user->id));
									...
									return $this->responseJsonError(Lang::get("messages.WrongEmailAddress"));
								}
							}
						}
				} else {
					return $this->responseJsonError(Lang::get("messages.WorkoutCannotBeShared"));
				}
			}
		}
		return $this->responseJson(Lang::get("messages.WorkoutShared"));
	}
...
//			        	Feeds::insertFeed("NewWorkoutSharedClient",Auth::user()->id,Auth::user()->firstName,Auth::user()->lastName);
			        	return $this->responseJson(Lang::get("messages.WorkoutSharedToClient")); 
			        }
...
//					Feeds::insertFeed("NewWorkoutShared",Auth::user()->id,Auth::user()->firstName,Auth::user()->lastName);
					...
					Event::fire('shareAWorkout', array(Auth::user(),""));
	...
	public function PrintWorkout($workoutId,$locale="en"){
		...
		// ]);
         Event::fire('printWorkout', array($user,$workout->name));
            
		return View::make("workoutPrint")
			->with("user",$user)
			->with("workout",$workout)
	...
	public function PrintWorkouts($workoutIds){
        ...
        Event::fire('printWorkouts',Auth::user());

        $user = Auth::user();
		return View::make("workoutPrints")
			->with("user",$user)
	...
	public function openWorkoutBySharingLink($link){
		...
		if(!Auth::check()){
			if($sharing->toUserObject){
				...
				if($invite){
					return View::make('TraineeSignUp')->with("key",$invite->key)->with("invite",$invite);
				} else {

					return Redirect::route("login");
				}
			} else {
				return View::make('TraineeSignUp');
			}
		}
		//DEFAULT AUTH
		...
		if($sharing){
			...
			if($sharing->type == "Workout"){
				...
				if($workout and $workout->userId == Auth::user()->id){
					...
					if(Auth::user()){
						return Redirect::route('Trainee', array('userName' => Helper::formatURLString(Auth::user()->firstName.Auth::user()->lastName)))->withError(Lang::get("messages.WorkoutNotFound"));
					} else{
						return Redirect::route("home")->withError(Lang::get("messages.WorkoutNotFound"));
					} 
				}
			}
		} else {
	...
	public function previewWorkout($workoutId,$workoutName="",$workoutAuthor=""){
		...
		if($workout){
			...
			if(Auth::check()){
							return View::make("workoutShare")
								->with("workout",$workout)
								->with("user",Auth::user())
								->with("sale",true)
								->with("exercises",$workout->getExercises()->get());
						} else {
							return View::make("workoutVisitor")
								->with("workout",$workout)
								->with("sale",true)
								->with("exercises",$workout->getExercises()->get());
						}
	...
	public function indexWorkoutMarket(){
		$user = Auth::user();
		if(Input::has("pageSize")) $this->pageSizeTrending = Input::get("pageSize") + $this->pageSizeTrending;
		return View::make("widgets.base.workoutMarket")
	...
	public function indexWorkoutMarketFull(){
		$user = Auth::user();
		if(Input::has("pageSize")) $this->pageSizeFullTrending = Input::get("pageSize") + $this->pageSizeFullTrending;
		return View::make("widgets.full.workoutMarket")
	...
	public function indexWorkoutTrainerFull(){
		...
		return View::make("widgets.full.workoutsTrainer")
	...
	public function indexWorkoutTrainer(){
		...
		return View::make("widgets.base.workoutsTrainer")
			->with("freeWorkouts",Workouts::forsaleFree()->where("authorId",$user->id)->take($this->pageSize)->get())
			->with("paidWorkouts",Workouts::forsalePaid()->where("authorId",$user->id)->take($this->pageSize)->get())
			->with("newWorkouts",Workouts::forsale()->where("authorId",$user->id)->take($this->pageSize)->orderBy("updated_at","DESC")->get())
			->with("freeTotal",Workouts::forsaleFree()->count())
	...
	public function indexWorkoutsClient(){
		...
		return View::make("widgets.base.workoutsClient")
			->with("countArchiveWorkouts",$countArchiveWorkouts)
			->with("archive",$archive)
			->with("client",$client)
			->with("workouts",$workouts)
	...
	public function indexWorkoutTrainee(){
		...
		return View::make("widgets.base.workoutsTrainee")
			->with("archive",$archive)
			->with("workouts",$workouts)
			->with("workoutsTotal",$workoutsTotal);
	}

	public function startWorkoutPerformance(){
		$workoutId = Input::get("workoutId");
		$performanceId = Input::get("performanceId");
		$workout = Workouts::find($workoutId);
	...
	public function discartOldPerformance(){
	...
	public function saveProgressPerformance(){
		$performanceId = Input::get("performanceId");
		$seconds = Input::get("totalSeconds");
		$performance = Workoutsperformances::find($performanceId);
		if($performance){
	...
	public function performWorkout(){
		$workoutId = Input::get("workoutId");
		$performanceId = Input::get("workoutPerformanceId");
		...
		if($performanceId == 0){
			...
			if($workout){
				...
				$performance->workoutId = $workout->id;
				$performance->forTrainer = $workout->authorId;
				$performance->timeInSeconds = (Input::get("totaltime") != "") ? Input::get("totaltime")*60 : Input::get("totalTime")*60;
				$performance->dateCompleted = Carbon::now();
				$performance->comments = Input::get("performanceComments");
				$performance->ratingId = Input::get("rating");
				$performance->save();

				$performance->notifyTrainerPerformance();

				return Redirect::route(strtolower(Auth::user()->userType)."Workouts")->with("message",Lang::get("messages.Your performance has been tracked"));

			} else {
				return Redirect::route(strtolower(Auth::user()->userType)."Workouts")->with("error",Lang::get("messages.Oops"));
			}
		} else {
			$performance = Workoutsperformances::find($performanceId);
			if($performance){
	...
	public function indexWorkoutsTrainer()
	{
		...
		return View::make("trainer.workouts");
	}

	public function clientWorkouts($id,$clientName)
	{
		...
		return View::make("trainer.clientWorkouts")
		->with("client",$client);
	}



	public function addCustomPicture(){
		$user = Auth::user();
		$workoutExercise = WorkoutsExercises::find(Input::get("workoutExercise"));
		$exerciseImage = new ExercisesImages();
		$exerciseImage->userId = $user->id;
		$exerciseImage->exerciseId = $workoutExercise->exerciseId;
		if(Input::has("availability")) $exerciseImage->availability = "public";
		$exerciseImage->save();

		Helper::checkUserFolder($user->id);
			if(Input::hasFile("customPicture")) {
					$images = Helper::saveImage(Input::file("customPicture"),$user->getPath().Config::get("constants.exercisesCustomPath")."/".$exerciseImage->id);
		...
		$data["message"] = Lang::get("messages.pictureAdded");
		$data["error"] = "";
	...
	public function exercisePerformance($workoutExericseId){
		...
		if ($sets->count() > 1){
       	...
       	return View::make("popups.performance")
	...
	public function saveEditWorkout(){
		$workoutId = Input::get("workoutId");
		$workout = Workouts::find($workoutId);
		$validation = Workouts::validate(Input::all());
		if($validation->fails()){
			
			return Redirect::to("/Workout/Edit/".$workoutId."/")->withErrors($validation->messages());
		} else {
		
		//$sets = Input::get("set_reps");

		$workout->description = Input::get("description");
		$workout->name = Input::get("name");
		//$workout->objectives = Input::get("objectives");
		if(Input::get("available") == "Yes") { $workout->availability = "public"; $workout->sale = 1; };
		if(Input::get("available") == "No") { $workout->availability = "private"; $workout->sale = 0; }
		//$workout->price = Input::get("price");
		//$workout->category = Input::get("category");
		$workout->save();
		return Redirect::to($workout->getURL())
				->with("message",Lang::get("messages.WorkoutSaved"));
		}
	}
	...
	public function viewWorkout($id,$name,$author){
		...
		if($workout){
			if($workout->status == "Draft"){
				return Redirect::to( Lang::get("routes./Trainer/CreateWorkout/").$workout->id);
			}
			$workout->incrementViews();
			...
			Event::fire('viewWorkout', array(Auth::user(),$workout->name));
	...
	public function viewWorkoutPDF($id,$name,$author){
		...
		if($workout){
			Event::fire('pdfWorkout', array(Auth::user(),$workout->name));
			$workout->incrementViews();
        ...
        if ($zip->open($zipFilePath, ZipArchive::OVERWRITE|ZipArchive::CREATE) === TRUE) {
			...
			foreach($workouts as $workout){
				...
				if($workout){
					...
					Event::fire('printWorkouts', array($user,$workout->name));
				}
				$counter++;
			}
	...
	public function createNewWorkout(){
		$userId = Auth::user()->id;
		$permissions = null;
		if(Input::has("userId")){
			$permissions = Helper::checkPremissions(Auth::user()->id,Input::get("userId"));
		...
		Event::fire('userNewWorkout', array(Auth::user()));

		return View::make("trainee.createWorkout")
			->with("permissions",$permissions)
			->with("total",Workouts::where("userId","=",$userId)->count());
	}


	public function createNewWorkoutTrainer($workoutId=""){
		...
		if($workoutId == ""){
			...
			if($create){
				...
				$workout->name = Input::get("workoutName");
		...
		return View::make("trainer.createWorkout")
			->with("workout",$workout)
			->with("permissions",$permissions)
			->with("tags",$tags)
	...
	public function editWorkout($workoutId="",$name="",$author="",$client=""){
		...
		// 	$workoutNew->name = Input::get("name");
		// 	$workoutNew->save();
		...
		if(!$permissions["edit"]){
            //Redirect::route("trainerWorkouts")->withError(Lang::get("messages.permissions"));
        }
    	} else {
    		Redirect::route("trainerWorkouts")->withError(Lang::get("messages.permissions"));
    	}
		...
		Event::fire('editAWorkout', array(Auth::user(),$workout->name));
	...
	public function createNewWorkoutTrainerToClient($clientId){
	...
	public function addSetsReturnTable(){
		$workoutExerciseId = Input::get("workoutExerciseId");
		...
		return View::make("widgets.full.workoutExercise")
	...
	public function addSets(){
		...
		$workoutExercise->updated_at = date("Y-m-d H:i:s");
		$workoutExercise->save();
		$workout = Workouts::find($workoutExercises->workoutId);
		$workout->createNewSetsExercise($workoutExerciseId);

	}

	public function saveSingleSet(){

		$set = Sets::find(Input::get("set"));
		$workoutExercise = WorkoutsExercises::find($set->workoutsExercisesId);
		$workout = Workouts::find($workoutExercise->workoutId);
		if($set){
			//if(Input::get("type") != "cardio"){
				//NOTIFY TRAINER IF WEIGHT CHANGES
				if(Input::get("weight") != $set->weight) {
				...
				if(Input::get("reps") != $set->reps and $workout->trainerMonitoringId != "" and Clients::checkIfTrainerHasClient($workout->trainerMonitoringId,$workout->userId,$workout->id)) { 
			...
			//} else {
				...
				if(Input::get("distance") != $set->distance and $workout->trainerMonitoringId != "" and Clients::checkIfTrainerHasClient($workout->trainerMonitoringId,$workout->userId,$workout->id)) { 
				...
				if(Input::get("time") != $set->time and $workout->trainerMonitoringId != "" and Clients::checkIfTrainerHasClient($workout->trainerMonitoringId,$workout->userId,$workout->id)) { 
				...
				if(Input::get("speed") != $set->speed and $workout->trainerMonitoringId != "" and Clients::checkIfTrainerHasClient($workout->trainerMonitoringId,$workout->userId,$workout->id)) { 
				...
				$set->distance = Input::get("distance");
				$set->bpm = Input::get("bpm");
				$set->time = Input::get("time");
				$set->speed = Input::get("speed");
				$set->completed = 1;
			//}
			$set->save();
			...
			if($set->last == 1){
				...
				return $this::responseJson(Lang::get("messages.NewSetsCreatedAndSaved"));
			}
			return $this::responseJson(Lang::get("messages.SetSaved"));
		} else {
			return $this::responseJsonError(Lang::get("messages.SetNotValid"));
		}
	}

	public function exerciseCompleted(){
		$user = Auth::user();
		$workoutsExercisesId = Input::get("workoutsExercisesId");
		...
		if(!Notifications::checkIfTrainerNotifiedTodayWorkout($user->id,null)){
			...
			if(count($trainers > 0)){
				foreach($trainers as $trainer){
					Notifications::insertDynamicNotification(Lang::get("messages.TraineeCompleted"),$trainer,$user->id,array(),false);
				}
			}
		}

        
		return $this::responseJson(Lang::get("messages.ExerciseCompleted"));
	}

	public function workoutCompleted(){
		$workoutId = Input::get("workoutId");
		$workout = Workouts::find($workoutId);
		if($workout){
			...
			return $this::responseJson(Lang::get("messages.WorkoutCompleted"));
		}
	}

	public function updateUnitExerciseGroup(){
		$id = Input::get("id");
	...
	public function saveAllSets(){
		...
		return $this::responseJson(Lang::get("messages.AllSetsSaved"));
	}

	public function saveAllAddNewSets(){
		$workoutId = Input::get("workoutId");
		$workoutExerciseId = Input::get("exerciseId");
		$sets = Input::get("sets");
		foreach($sets as $set){
			$setFetched = Sets::find($set["idSet"]);
			if($setFetched){
	...
	public function createNewWorkoutAddEdit(){
		$userId = Auth::user()->id;
		$permissions = null;
		if(Input::has("userId")){
			$permissions = Helper::checkPremissions(Auth::user()->id,Input::get("userId"));
		...
		$workoutDetails = json_decode(stripslashes(Input::get("program_details")),true);
		$workout = new Workouts();
		$workout->name = $workoutDetails["name"];
		$workout->shares = 0;
		$workout->views = 0;
		$workout->timesPerformed = 0;
		$workout->objectives = Input::get($workoutDetails["obj"]);
		$workout->userId = $userId;
		$workout->authorId = $userId;
		$workout->availability = "private";
		$workout->version = Config::get("constants.version");
		...
		$exercises = json_decode(stripslashes(Input::get("program")),true);
		...
		return Redirect::route("traineeWorkouts")
			->with("message",Lang::get("messages.WorkoutCreated"))
			->with("permissions",$permissions)
			->with("total",Workouts::where("userId","=",$userId)->count());
	}


	public function autoSaveWorkout(){

		$workout = null;
		$userId = Auth::user()->id;
		$flag = false;
		if(Input::has("id") and Input::get("id") !=  ""){
			$workout = Workouts::find(Input::get("id"));
		} else {
			$workout = new Workouts();
			$flag = true;
		}
		...
		$workout->exercises = Input::get("exercises");
		$workout->exerciseGroup = Input::get("exerciseGroup");
		$workout->exerciseGroupRest = Input::get("exerciseGroupsRest");
		$workout->save();
	...
	public function duplicateWorkout(){
		...
		if($workout){
			...
			Event::fire('duplicateWorkout', array(Auth::user(),$workout->name));

			return Redirect::route("trainerWorkouts")->with("message",Lang::get("messages.WorkoutHasBeenDuplicated"));

		} else {

			return Redirect::route("trainerWorkouts")->withError(Lang::get("messages.WorkoutCannotBeDuplicated"));
		}
	}

	public function createNewWorkoutAddEditTrainer(){
		...
		$groups = json_decode(Input::get("exerciseGroup"),true);
		$groupsRest = json_decode(Input::get("exerciseGroupsRest"),true);
		...
		foreach($groups as $group){
			...
			foreach($group as $exercise){
					...
					$workoutExercise->order = $order;
					$workoutExercise->notes = $exercise["notes"];
					$workoutExercise->tempo1 = (array_key_exists("tempo1", $exercise)) ? $exercise["tempo1"] : "";
					$workoutExercise->tempo2 = (array_key_exists("tempo2", $exercise)) ? $exercise["tempo2"] : "";
					$workoutExercise->tempo3 = (array_key_exists("tempo3", $exercise)) ? $exercise["tempo3"] : "";
					$workoutExercise->tempo4 = (array_key_exists("tempo4", $exercise)) ? $exercise["tempo4"] : "";
		...
		if($client != 0 and $client != ""){
			...
			if($client){
					...
					->with("message",Lang::get("messages.CreatedAndShared"))
					->with("permissions",$permissions);
			} else {
	...
	public function assignWorkoutToClient($client,$workoutId){
		$permissions = null;
		if(Input::has("userId")){
			$permissions = Helper::checkPremissions(Auth::user()->id,Input::get("userId"));
			if($permissions["view"]){
...
//			Feeds::insertFeed("DeletedWorkout",Auth::user()->id,Auth::user()->firstName,Auth::user()->lastName);
			...
			Event::fire('deleteAWorkout', array(Auth::user()));
            
			return $this::responseJson(Lang::get("messages.WorkoutDeleted"));
		} else {
	...
	public function archiveWorkout($id)
	{
		...
		Event::fire('archiveWorkout', array(Auth::user(),$obj->name));
        
		return $this::responseJson(Lang::get("messages.WorkoutArchived"));
	...
	public function unarchiveWorkout($id)
	{
		...
		Event::fire('unArchiveWorkout', array(Auth::user(),$obj->name));
        
		return $this::responseJson(Lang::get("messages.WorkoutUnArchived"));
	...
	public function deleteWorkout($id)
	{
		...
		if(!$obj) return Redirect::route("trainerWorkouts")->withError(Lang::get("messages.NotFound"));
...
//			Feeds::insertFeed("DeletedWorkout",Auth::user()->id,Auth::user()->firstName,Auth::user()->lastName);
			...
			return Redirect::route("trainerWorkouts")->with("message", Lang::get("messages.WorkoutDeleted")); 
		} else {
			return Redirect::route("trainerWorkouts")->withError(Lang::get("messages.Permissions")); 
		}
	...
	public function APIsaveSingleSet(){
		...
		if($set){
			...
			if($set->last == 1){
				...
				$result = Helper::APIOK();
				$result["message"] = Lang::get("messages.NewSetsCreatedAndSaved");
				return $result;

			}
			$result = Helper::APIOK();
			$result["message"] = Lang::get("messages.SetSaved");
			return $result;
		} else {
			$result = Helper::APIERROR();
			$result["message"] = Lang::get("messages.SetNotValid");
			return $result;
		}
	}

	public function APIcompleteSet(){
		$set = Sets::find(Input::get("id"));
	...
	public function APIexerciseCompleted(){
		$user = Auth::user();
		$workoutsExercisesId = Input::get("workoutsExercisesId");
		$workoutExercise = WorkoutsExercises::find($workoutsExercisesId);
		//$sets = Input::get("sets");
		...
		foreach($sets as $set){
			...
			if($setFetched){
				...
				$result = Helper::APIERROR();
				$result["message"] = Lang::get("messages.NotFound");
				return $result;
		...
		$result = Helper::APIOK();
		$result["message"] = Lang::get("messages.ExerciseCompleted");
		return $result;

	}

	public function APIworkoutCompleted(){
		$user = Auth::user();
		$workoutId = Input::get("workoutId");
		$workout = Workouts::find($workoutId);
		if($workout){
			...
			$result = Helper::APIOK();
			$result["message"] = Lang::get("messages.WorkoutCompleted");
			return $result;
		}	
	}

	public function APIsaveAllSets(){
		...
		$result = Helper::APIOK();
		$result["message"] = Lang::get("messages.AllSetsSaved");
		return $result;
	...
	public function API_Workout_Groups(){
		$workoutId = Input::get("id");
		$workout = Workouts::find($workoutId);
		$user = Auth::user();
		if($workout){
			if($workout->status == "Draft"){
