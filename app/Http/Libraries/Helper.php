...
class Helper
{
    ...
    {
        // Trim and basic format validation
        $email = trim($email);
        if ($email === '') return false;

        // Check for invalid characters, empty domain or local part
        ...
        // No leading or trailing dots in local or domain part
        [$local, $domain] = explode('@', $email);
        if ($local === '' || $domain === '') return false;
        if (str_starts_with($local, '.') || str_ends_with($local, '.')) return false;
        if (str_starts_with($domain, '.') || str_ends_with($domain, '.')) return false;
    ...
    {
        return self::clean($email);
    }

    public static function clean(string $email): string
    {
        $email = trim($email);
        $email = mb_convert_encoding($email, 'UTF-8', 'UTF-8'); // remove invalid utf-8
        $email = preg_replace('/\s+/', '', $email);             // remove spaces and tabs
        $email = str_replace('..', '.', $email);                // remove double dots
        $email = strtolower($email);                            // normalize case
