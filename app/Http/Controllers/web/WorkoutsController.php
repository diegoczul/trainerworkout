...
class WorkoutsController extends BaseController
{
	...
	public function indexCreate(Request $request)
	{
		...
		if ($request->has("arrayData")) {
			...
			if (is_array($arrayData)) {
				if (array_key_exists("search", $arrayData) and $arrayData["search"] != "") $search = $arrayData["search"];
				if (array_key_exists("archive", $arrayData) and $arrayData["archive"] == "true") $archive = true;
				if (array_key_exists("addWorkout", $arrayData) and $arrayData["addWorkout"] == "true") {
					$addWorkout = true;
					$click = false;
				}
			}
	...
	public function subscribeTrainer(Request $request)
	{
		...
		if ($workout) {
			if ($request->get("subscribeToWorkout") == "true") {
				...
				$updates = UserUpdates::where("trainerId", Auth::user()->id)->where(function ($query) use ($workout, $childWorkouts) {
					$query->orWhere("auxId", $workout->id);
					$query->orWhereIn("auxId", $childWorkouts);
				})->where("type", "workout")->get();
		...
		foreach ($exercises as $exercise) {
			...
			foreach ($exercise["reps"] as $rep) {
				...
				if (array_key_exists($index, $exercise["time"]) and  $exercise["time"][$index] != "") {
					...
					if (array_key_exists($index, $exercise["weight"])) {
						$templateSet->weight = $exercise["weight"][$index];
						$templateSet->weightKG = Helper::formatWeight($exercise["weight"][$index] / 2.2);
					};
					if (array_key_exists($index, $exercise["templateWeight"])) {
						$templateSet->weight = $exercise["templateWeight"][$index];
						$templateSet->weightKG = Helper::formatWeight($exercise["templateWeight"][$index] / 2.2);
					};
	...
	public function API_DownloadWorkout(Request $request)
	{
		...
		$validation = Validator::make($request->all(), [
			'workout_id' => ['required', Rule::exists('workouts', 'id')->whereNull('deleted_at')],
			'include_pdf' => [Rule::requiredIf(function () use ($request) {
				return (!$request->filled('include_image') || $request->get('include_image') == false);
			}), 'required_if:include_image,false', 'boolean'],
			'include_image' => [Rule::requiredIf(function () use ($request) {
				return (!$request->filled('include_pdf') || $request->get('include_pdf') == false);
			}), 'required_if:include_pdf,false', 'boolean'],
		]);
		if ($validation->fails()) {
			$result["message"] = $validation->messages()->first();
			return $this::responseJsonError($result);
